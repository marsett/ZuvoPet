...
using ZuvoPet.Models;

namespace ZuvoPet.Repositories
{
    public class RepositoryZuvoPet : IRepositoryZuvoPet
    {
        ...
        public async Task<Usuario> LogInUserAsync(string nombreUsuario, string contrasena)
        {
            ...
            {
                var consultamario = from datos in this.context.Usuarios
                               where datos.NombreUsuario == nombreUsuario
                               select datos;
                Usuario usermario = await consultamario.FirstOrDefaultAsync();

                // Si el usuario existe en la base de datos, lo devolvemos sin verificar contraseña
                if (usermario != null)
                {
                    return usermario;
                }
            }
            // Excepción para usuario refugio_vida
            if (nombreUsuario == "refugio_vida" && contrasena == "890")
            {
                var consultarefugio = from datos in this.context.Usuarios
                                    where datos.NombreUsuario == nombreUsuario
                                    select datos;
                Usuario userrefugio = await consultarefugio.FirstOrDefaultAsync();
            ...
            // Excepción para usuario hogar_animal
            if (nombreUsuario == "hogar_animal" && contrasena == "345")
            {
                var consultarefugio = from datos in this.context.Usuarios
                                      where datos.NombreUsuario == nombreUsuario
                                      select datos;
